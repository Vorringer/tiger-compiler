...
static AS_instrList cat_instr(AS_instrList a, AS_instrList b) {
  if (a==NULL) return b;
  else return AS_InstrList(a->head, cat_instr(a->tail, b));
}


AS_instrList intersect_instr(AS_instrList l1, AS_instrList l2)
{
    AS_instrList r=NULL;
    AS_instrList ll1=l1;
    AS_instrList ll2=l2;
...
AS_instrList union_instr(AS_instrList l1, AS_instrList l2)
{
    AS_instrList l=cat_instr(l1,l2);
    AS_instrList ll=l;
    for (; l; l = l->tail) 
    {
        AS_instrList tmpl = l->tail;
        AS_instr tmp = l->head;
        AS_instrList parent = l;
...
AS_instrList delete_instr(AS_instr instr, AS_instrList mlist)
{
  assert(instr && mlist);
  if (instr==mlist->head) return mlist->tail;
  else return AS_InstrList(mlist->head, delete_instr(instr, mlist->tail));
}

bool inASinstrList(AS_instr instr, AS_instrList l) {
  AS_instrList p;
  for(p=l; p!=NULL; p=p->tail)
    if (p->head==instr) return TRUE;
  return FALSE;
}
